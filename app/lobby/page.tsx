...
export default function LobbyPage() {
  ...
  if (loading || !user) return <div className="h-screen flex items-center justify-center bg-[var(--background)]"><div className="animate-pulse text-[var(--color-secondary)] font-bold uppercase tracking-widest">Loading...</div></div>;

  return (
    <div className="h-full w-full p-6 max-w-7xl mx-auto flex flex-col gap-6">

      {/* HEADER */}
      <Panel className="flex items-center justify-between shrink-0 py-4 px-6 bg-[var(--panel-bg)] border-[var(--border)]">
        <div>
          ...
          <p className="text-[var(--color-secondary)] text-xs font-bold flex items-center gap-2 tracking-wide">
        ...
        <div className="flex gap-4">
          {/* ELO Display */}
          <div className="hidden md:flex items-center gap-2 bg-[var(--card-bg)] px-4 py-2 rounded-xl border border-[var(--border)] shadow-inner">
            <div className="flex flex-col items-center">
              <span className="text-[9px] uppercase font-bold text-[var(--color-secondary)] tracking-wider">Blitz</span>
              <span className="text-base font-black text-[var(--foreground)] font-mono">{profile?.elo_blitz || 800}</span>
            </div>
            <div className="w-px h-8 bg-[var(--border)]" />
            <div className="flex flex-col items-center">
              <span className="text-[9px] uppercase font-bold text-[var(--color-secondary)] tracking-wider">Rapid</span>
              <span className="text-base font-black text-[var(--foreground)] font-mono">{profile?.elo_rapid || 800}</span>
            </div>
          </div>
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 min-h-0">
        ...
        {/* LEFT: CHALLENGE LIST */}
        <GameCard variant="default" className="lg:col-span-8 p-0 flex flex-col overflow-hidden bg-[var(--card-bg)] border-[var(--border)]">
          <div className="p-4 border-b border-[var(--border)] bg-[var(--header-bg)] flex justify-between items-center shadow-md z-10">
            <h2 className="text-sm font-black text-[var(--foreground)] font-display tracking-wide flex items-center gap-2">
              <Zap size={16} className="text-amber-400 fill-amber-400" /> Active Games
            </h2>
            <span className="text-[10px] font-bold bg-[var(--panel-bg)] text-[var(--color-secondary)] px-3 py-1 rounded-full border border-[var(--border)] shadow-inner">
              {challenges.length} Online
            </span>
          </div>

          <div className="flex-1 overflow-y-auto scrollbar-subtle p-4 space-y-3 bg-[var(--background)]/50">
            <AnimatePresence>
              {challenges.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-[var(--color-secondary)] space-y-4">
                  <div className="w-24 h-24 rounded-full bg-[var(--panel-bg)] flex items-center justify-center animate-pulse border-4 border-[var(--border)]">
                    <Swords size={40} className="text-[var(--color-secondary)]" />
                  </div>
                  <div className="text-center">
                    <p className="font-black text-[var(--color-secondary)] font-display tracking-wide text-lg">The Arena is Quiet</p>
                    <p className="text-xs text-[var(--color-muted)] tracking-wider mt-1 font-bold">Be the first to start a battle</p>
                  </div>
                </div>
              ) : (
                challenges.map((challenge) => (
                  <motion.div
                    ...
                    className="p-1 rounded-2xl bg-gradient-to-b from-[var(--border)]/50 to-transparent hover:from-amber-500/10 transition-all duration-300"
                  >
                    <div className="bg-[var(--panel-bg)] p-4 rounded-xl flex items-center justify-between border border-[var(--border)] hover:border-amber-500/50 transition-colors shadow-lg">
                      <div className="flex items-center gap-4">
                        {/* Host Avatar */}
                        <div className="w-14 h-14 rounded-xl bg-[var(--card-bg)] border-2 border-[var(--border)] overflow-hidden relative shadow-inner">
                          {challenge.host?.avatar_url ? (
                            ...
                            <div className="w-full h-full flex items-center justify-center bg-[var(--panel-bg)]">
                              <div className="text-xl font-black text-[var(--color-secondary)] select-none">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            <h3 className="font-black text-[var(--foreground)] font-display tracking-wide text-base group-hover:text-amber-400 transition-colors">
                          ...
                          <div className="flex items-center gap-2 mt-2">
                            <span className="text-[10px] font-bold text-[var(--color-secondary)] flex items-center gap-1 bg-[var(--card-bg)] px-2 py-1 rounded border border-[var(--border)] uppercase tracking-wider">
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          ...
          {/* Arena Map Preview */}
          <GameCard variant="blue" className="flex-1 flex flex-col bg-[var(--card-bg)] p-0 overflow-hidden">
            <div className="p-4 border-b border-[var(--border)] bg-blue-950/20 flex justify-between items-center">
            ...
            <div className="flex-1 relative bg-[var(--background)] shadow-inner p-4">
              {progress.blitz ? (
                <ArenaPath
                ...
                <div className="h-full flex items-center justify-center text-[var(--color-secondary)] text-xs font-bold uppercase tracking-widest">
                  Loading Arena...
                </div>
              )}
            </div>
          ...
          {/* Chests */}
          <GameCard variant="gold" className="shrink-0 bg-[var(--card-bg)] p-0 overflow-hidden">
            <div className="p-4 border-b border-[var(--border)] bg-amber-950/20">
      ...
      <Dialog open={!!selectedArena} onOpenChange={(open) => !open && setSelectedArena(null)}>
        <DialogContent className="glass-panel bg-[var(--panel-bg)]/95 border-[var(--border)] text-[var(--foreground)] max-w-4xl h-[80vh] flex flex-col p-0 overflow-hidden backdrop-blur-xl">
