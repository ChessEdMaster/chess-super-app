    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-4 relative z-10">
      <GameCard className="w-full max-w-md flex flex-col items-center text-center p-8 bg-zinc-900/95 border-amber-600/50 shadow-2xl">
        <div className="mb-6 relative">
          ...
          <div className="w-20 h-20 bg-gradient-to-br from-amber-400 to-amber-600 rounded-2xl flex items-center justify-center border-t-2 border-amber-300 shadow-[0_4px_0_#b45309] relative z-10 rotate-3">
            <Trophy size={40} className="text-white drop-shadow-md" />
          </div>
        </div>

        <h1 className="text-3xl font-black text-white mb-2 font-display uppercase tracking-wide text-stroke">Benvingut</h1>
        <p className="text-amber-100/60 mb-8 font-medium">Inicia sessió per entrar a l'Arena</p>

        {/* Email/Password Form */}
        <form onSubmit={handleEmailLogin} className="w-full mb-6 space-y-4">
          <div className="relative group">
            <Mail className="absolute left-4 top-1/2 transform -translate-y-1/2 text-zinc-500 group-focus-within:text-amber-400 transition-colors" size={20} />
            <input
              type="email"
              ...
              required
              className="w-full bg-black/40 text-white pl-12 pr-4 py-4 rounded-xl border-2 border-zinc-700 focus:border-amber-500 focus:outline-none transition-all shadow-inner font-bold placeholder:font-normal placeholder:text-zinc-600"
            />
          </div>
          <div className="relative group">
            <Lock className="absolute left-4 top-1/2 transform -translate-y-1/2 text-zinc-500 group-focus-within:text-amber-400 transition-colors" size={20} />
            <input
              type="password"
        ...
        <div className="text-zinc-400 text-sm mt-8 font-medium">
          No tens compte? <Link href="/register" className="text-amber-400 hover:text-amber-300 font-bold hover:underline">Registra't aquí</Link>
        </div>
