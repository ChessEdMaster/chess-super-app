...
export default function MapaEscacsContainer({ initialLocations }: MapaEscacsContainerProps) {
    ...
    return (
        <div className="flex flex-col h-full gap-6 p-4 max-w-[1920px] mx-auto">
            {/* Header */}
            <Panel className="flex items-center gap-4 py-4 px-6 bg-[var(--header-bg)] border-[var(--border)]">
            ...
            <div className="flex flex-col lg:flex-row gap-6 flex-1 min-h-0">
                ...
                <div className="w-full lg:w-1/3 flex flex-col gap-6">
                    {/* 1. Panell de Filtres */}
                    <GameCard variant="default" className="p-4 bg-[var(--card-bg)]/80 border-[var(--border)]">
                        <FiltresMapa
                            locations={locations}
                    ...
                    <GameCard variant="default" className="flex-1 min-h-[400px] bg-[var(--card-bg)]/80 border-[var(--border)] flex flex-col overflow-hidden">
                        <div className="p-4 border-b border-[var(--border)] bg-[var(--panel-bg)]/30">
                            <h2 className="text-sm font-black text-[var(--color-secondary)] uppercase tracking-wider">Llistat ({filteredLocations.length})</h2>
                        </div>
                ...
                <div className="w-full lg:w-2/3 flex flex-col">
                    {/* 2. Secció del Mapa */}
                    <GameCard variant="blue" className="flex-1 w-full min-h-[600px] lg:min-h-0 rounded-2xl shadow-2xl overflow-hidden relative z-0 border-4 border-[var(--border)] p-0">
                        <MapaEscacs
                            locations={locations}
